  ...
  return (
    <div className="w-full flex flex-col p-0 text-xs">
      <div className="flex justify-between items-center mb-2">
        <button onClick={goToPrevMonth} className="p-0.5 rounded">&#8592;</button>
        <h2 className="text-sm font-bold">{format(currentDate, "MMMM yyyy")}</h2>
        <button onClick={goToNextMonth} className="p-0.5 rounded">&#8594;</button>
      </div>
      <div className="grid grid-cols-7 gap-y-0.5 gap-x-1">
        ...
        {days.map((day) => (
          <div
            key={day.toISOString()}
            className={`p-0.5 text-center border-none rounded ${isSameDay(day, today) ? 'bg-blue-500 text-white' : ''}`}
          >
            {format(day, "d")}
          </div>
        ))}
